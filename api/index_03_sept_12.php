...
class Api
{
	public function __construct()     
	{
		$this->requestText  = array('TWILIOAPI_LOGIN','TWILIOAPI_REGISTRATION','TWILIOAPI_FORGOTPASSWORD','TWILIOAPI_USERLIST','TWILIOAPI_SENDVOICESMS',
									...
									'TWILIOAPI_AUTH_RESEND','TWILIOAPI_TEXTMSG_INBOX','TWILIOAPI_USERCONTACTS_IMPORTS');
	...
	*/
	private function loginUser()
	{
		...
		if($mobileNo!='' && $pass!='')
		{
				$result = $_objDB->ExecuteMultiQuery("SELECT id, mobileNo,status FROM tblUsers WHERE mobileNo='".$mobileNo."' AND password='".base64_encode($pass)."'");
				...
				if(!empty($data))
				{
					...
					{
						$userId = $data['0']['id'];
						$msg = array("Message"=>"Active","UserId"=>$userId,"UserName"=>$data['0']['mobileNo'],"Status"=>"ok");				
					...
					{
						$msg = array("Message"=>"Deactive","UserId"=>$data['0']['id'],"loginStatus"=>$data['0']['status'],"Status"=>"Failed");
					}
				}
				else
				{
	   ...
	   $userId  = trim(Request::data('userId'));
		if(!empty($userId) && is_numeric($userId))
		{
			...
			if(!empty($usrList))
			{
				...
				{
					$time = $this->timeDiff(strtotime($row["dateAdded"]));
					$msg[] = array("Message"=>"Success","txtMsgId"=>$row['txtMsgId'],"txtMessage"=>$row['Message'], "dateAdded"=>$time, "Status"=>"Ok"); 
				}
			}
			else
			{
  ...
  private function userContactImport()
  {
	...
	if($uid!='' && is_array($mobile) && is_array($nameE))
	{			
