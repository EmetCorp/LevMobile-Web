...
class Api
{
	public function __construct()     
	{
		$this->requestText  = array('TWILIOAPI_LOGIN','TWILIOAPI_REGISTRATION','TWILIOAPI_FORGOTPASSWORD','TWILIOAPI_USERLIST','TWILIOAPI_SENDVOICESMS',
									...
									'TWILIOAPI_AUTH_RESEND','TWILIOAPI_TEXTMSG_INBOX','TWILIOAPI_TEXTMSG_OUTBOX','TWILIOAPI_USERCONTACTS_IMPORTS',
									'TWILIOAPI_PLACE_A_CALL','TWILIOAPI_AUTHORIZE_NET');
	...
	public function processRequest()
	{
		...
		{                       
			...
			break; 
			case 'TWILIOAPI_TEXTMSG_OUTBOX':						
				return $this->getTextOutbox();
			break;
			...
			break;	
			case 'TWILIOAPI_AUTHORIZE_NET':
				return $this->authorizaNet();
			break;
		...
		if($mobile!='')
		{
				...
				{					
					$mobmsg = "Your authentication code : ".$data[0]['activeCode'].". Please enter your auth code to activate your account";
			  		 sms::sendSms("16465536390",$mobile,$mobmsg);
  ...
  private function registration()
		...
		if($mobileNo!='' && $AreaCode!='' && is_numeric($AreaCode))
		{
		   ...
		   elseif(!empty($checkEmail))
			...
			else
			{
				...
				$activeCode = Utility::getRandomNumeric(4);
				...
				if(!empty($LastId))
				  {
					...
					$subaccount = $client->accounts->create(array("FriendlyName" => $local_user['email']));
						  ...
						  /* Code to purchase a number for subaccount starts here */
							...
							if(empty($PhoneNumber))
							...
							else
							 {
								...
								/* Code to create Twilio application stsrts here */
								// Your Account Sid and Auth Token from twilio.com/user/account
															
								$client = new Services_Twilio($local_user['twilio_account_sid'], $local_user['twilio_auth_token']);
									
								$url = "http://www.levmobile.com/api/hello-client-monkey-twimll.php?callerId=".$PhoneNumber."&PhoneNumber=".$firstName;
									
									$app = $client->account->applications->create("PhoneMe", array(
									"VoiceUrl" => "$url",
									"VoiceMethod" => "GET"
									));
									$appSid = $app->sid;
									
									/**Update subaccount phone number code*/
									$number = $subaccount_client->account->incoming_phone_numbers->get($number->sid);
									$number->update(array("VoiceApplicationSid" => "$appSid","SmsApplicationSid" => "$appSid"));
								...
								if(empty($appSid))
								  ...
								  {									
									$_objDB ->ExecuteMultiQuery("UPDATE tblUsers SET appName='PhoneMe',  twimlapplication='".$appSid."' WHERE id='".$LastId."'");									  
								  }
								
								
							   /* Code to create Twilio application ends here */							
	...
	/*
	* User Patrons registration for the both Android/Iphone
	* http://twilio.demos.classicinformatics.com/api/?keyword=TWILIOAPI_CALLLOG&uid={UID}			
	*/
  private function callLog()
	{
		...
		{
			$_objDB->ExecuteMultiQuery("SELECT subaccountSid,subaccountAuthToken FROM tblUsers WHERE id='".$uid."'");
			$subAccount = $_objDB ->FetchRecords();
			$msg = $this->userLogDetails($subAccount[0]["subaccountSid"],$subAccount[0]["subaccountAuthToken"]);
		}
		else
		{
			$msg[] = array("Message"=>"Error","ErrorMsg"=>"You have missing user id"); 
		}
	...
	/*
	* User Patrons registration for the both Android/Iphone
	* http://twilio.demos.classicinformatics.com/api/?keyword=TWILIOAPI_AUTHORIZE_NET&uid={UID}&card_num={CARD}&exp_date={EXP}
	*/
	private function authorizaNet()
	{
		global $_objDB;
		$uid      = Request::data('uid');	
		$card_num = trim(Request::data('card_num'));	
		$exp_date = trim(Request::data('exp_date'));	
		if($uid!="" && is_numeric($uid) && $card_num!="" && $exp_date!="")
		{
			require_once 'authorizenet/AuthorizeNet.php'; 
			
			define("AUTHORIZENET_API_LOGIN_ID", "3wXM98bU"); 
			define("AUTHORIZENET_TRANSACTION_KEY", "2AL8duX9cG39L58B"); 
			...
			{
				$_objDB->ExecuteMultiQuery("UPDATE tblUsers SET userType='P' WHERE id='".$uid."'");
				$msg = array("Message"=>"You have made payment sucessfully","Status"=>"Ok"); 
			}
			else
			{
				$msg = array("Message"=>"Payment Declined by Authorize.net","Status"=>"Failed");
			}
		}
		else
		{
			$msg = array("Message"=>"You have missing user id","Status"=>"Failed"); 
		}
		echo json_encode($msg);
	}
  ...
  private function getTextInbox()
	...
	/*
	* get User out text message
	* http://twilio.demos.classicinformatics.com/api/?keyword=TWILIOAPI_TEXTMSG_OUTBOX&userId={userId}				
	*/
  private function getTextOutbox()
	{
	   global $_objDB;		
	   $userId  = trim(Request::data('userId'));
		if(!empty($userId) && is_numeric($userId))
		{
			/*$query = "SELECT * FROM tblTextSms WHERE toId='".$userId."' ORDER BY txtMsgId desc";
			*/
			$query = "SELECT TextSms.*, Users.firstName, Users.lastName FROM tblTextSms As TextSms LEFT JOIN tblUsers AS Users ON TextSms.toId = Users.id  WHERE fromId='".$userId."' ORDER BY txtMsgId desc";
			$_objDB ->ExecuteMultiQuery($query);
			$usrList  = $_objDB ->FetchRecords();
			if(!empty($usrList))
			{
  ...
  private function placeACall()
  {
	...
	/**Get user CALL/SMS Log*/
	protected function userLogDetails($subSid,$subAuthToken)
	{
		$version = '2010-04-01';
		$flag = true;
		// Set our AccountSid and AuthToken //
		$client = new Services_Twilio($subSid, $subAuthToken, $version);
		//$client->account->calls->getIterator(0, 50, array("Status" => "completed"))
		
		try 
		{
			if(count($client->account->calls) > 0)
			{
				foreach ($client->account->calls as $call) 
				{
					...
					$duration = "";
					$cost = "";	 
					if(!is_null($call->price))   $cost = $call->price;	 
					if(!is_null($call->duration))   $duration = $call->duration;	  
					$data[] = array("Message"=>"Success","Direction"=>$call->direction,"CallFrom"=>$call->from ,"CallTo"=>$call->to,
									"Call_staus"=>$call->status,"Cost"=>$cost,"StartTime"=>$call->start_time,"Duration"=>$duration);
				}
			}
			else
			{
				$flag = false;
				$data[] = array("Message"=>"Error","ErrorMsg"=>"No call log found"); 
			}
		...
		{
			$flag = false;
			$data[] = array("Message"=>"Error","ErrorMsg"=>$e->getMessage()); 
		}
		if($flag)
		{
			$data[] = array("Message"=>"Error","ErrorMsg"=>"Invalid sub id and Token"); 
		}
		return $data;
	}
