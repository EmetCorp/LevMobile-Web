...
class Api
{
	...
	*/
	private function loginUser()
	{
		...
		if($mobileNo!='' && $pass!='')
		{    
				$result = $_objDB->ExecuteMultiQuery("SELECT id, mobileNo,status, purPhoneNo FROM tblUsers WHERE mobileNo='".$mobileNo."' AND password='".base64_encode($pass)."'");
				...
				if(!empty($data))
				{
					...
					{
						$userId = $data['0']['id'];
						$msg = array("Message"=>"Active","UserId"=>$userId,"UserName"=>$data['0']['mobileNo'],"purPhoneNo"=>$data['0']['purPhoneNo'],"Status"=>"ok");				
					...
					{
						$msg = array("Message"=>"Deactive","UserId"=>$data['0']['id'],"purPhoneNo"=>$data['0']['purPhoneNo'],"loginStatus"=>$data['0']['status'],"Status"=>"Failed");
					}
				}
				else
				{
