...
class Api
{
	public function __construct()     
	{
		...
		$this->requestText = array('TWILIOAPI_LOGIN','TWILIOAPI_REGISTRATION','TWILIOAPI_FORGOTPASSWORD','TWILIOAPI_USERLIST','TWILIOAPI_SENDVOICESMS','TWILIOAPI_RECEIVEDVSMS','TWILIOAPI_UPDATEREADSTATUS','TWILIOAPI_CALLLOG');
...
private function forgotpass()
	...
	* http://twilio.demos.classicinformatics.com/api/?keyword=TWILIOAPI_REGISTRATION&mobileNo={mobileNo}&email={EMAIL}&password={PASSWORD}&firstName={firstName}&lastName={lastName}&address={address}&deviceId=(DEVICEID)&deviceToken={DEVICETOKEN}&deviceType={iphone}				
	*/
		...
		if($mobileNo!='')
		{
		   ...
		   elseif(!empty($checkEmail))
			...
			else
			{
				...
				$_objDB ->ExecuteMultiQuery("INSERT INTO tblUsers SET mobileNo='".$mobileNo."' , firstName='".$firstName."' , lastName='".$lastName."' , email='".$email."', password='".$password."', address='".$address."', dateAdded=now()");	
  ...
  /*
	* User USER LIST for the both Android/Iphone
	* http://twilio.demos.classicinformatics.com/api/?keyword=TWILIOAPI_SENDVOICESMS&fromId={fromId}&toId={toId}&vSmsFile={vSmsFile}				
	*/
  private function sendVoiceSms()
	{
		...
		$query  = "INSERT INTO tblVoiceSms(fromId, toId, dateAdded) VALUES('".$fromId."', '".$toId."', now())";
		$_objDB ->ExecuteMultiQuery($query);
		$LastId = $_objDB->InsertedID();
		...
		if($LastId)
		  {
			...
			if(isset($_FILES["vSmsFile"]["name"]) && $_FILES["vSmsFile"]["error"]=="0")
					...
					if(upload_my_file($_FILES["vSmsFile"]["tmp_name"],"..".$tmpPict))/**/
						{
						  ...
						  $query = "UPDATE tblVoiceSms SET vSmsFile='".$tmpPict."'  WHERE id='".$LastId."'";
			  			  $_objDB ->ExecuteMultiQuery($query);						
						  $msg = array("Message"=>"sent_Successfully","vSmsId"=>$LastId,"Status"=>"Ok");		
						}
					  else
					    {
							$query = "DELETE FROM tblVoiceSms WHERE id='".$LastId."'";
			  			    $_objDB ->ExecuteMultiQuery($query);	
							$msg = array("Message"=>"error_sending_voice_sms1111","Status"=>"Failed");	
						}
					}
				   else
					 {
							$query = "DELETE FROM tblVoiceSms WHERE id='".$LastId."'";
			  			    $_objDB ->ExecuteMultiQuery($query);	
							$msg = array("Message"=>"error_sending_voice_sms222","Status"=>"Failed");	
					 }
			...
			else
				{
						...
						$msg = array("Message"=>"error_sending_voice_sms333","Status"=>"Failed");	
				}				     
  ...
  private function receivedVoiceSms()
	{
	   ...
	   if(!empty($userId) && is_numeric($userId))
	    {
		 	$query = "SELECT SQL_CALC_FOUND_ROWS VoiceSms.id, VoiceSms.fromId, VoiceSms.vSmsFile, VoiceSms.readStatus, VoiceSms.dateAdded, Users.firstName,  Users.lastName FROM tblVoiceSms AS VoiceSms INNER JOIN tblUsers AS Users ON VoiceSms.fromId = Users.id WHERE VoiceSms.toId='".$userId."'";
		   ...
		   if(!empty($usrList))
			{
				foreach($usrList as $row)
				{
					...
					$msg[] = array("Message"=>"Success","ID"=>$row['id'], "firstName"=>$row["firstName"], "lastName"=>$row["lastName"], "vSmsFile"=>$vSmsFile, "readStatus"=>$row["readStatus"], "dateAdded"=>$row["dateAdded"], "Status"=>"Ok"); 
